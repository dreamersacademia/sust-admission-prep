...
export async function getOrPublishAnalytics(examId, questions) {
  ...
  // THE actual bug fix — see finalizeAllOverdueForExam's comment. If this
  // finds and fixes any straggler, we deliberately do NOT trust whatever
  // was cached before (it was built without them) — falling through to
  // a full fresh recompute below instead.
  const finalizedCount = await finalizeAllOverdueForExam(examId);

  const boardSnap = await boardRef.get();
  if (finalizedCount === 0 && boardSnap.exists && boardSnap.data().published) {
...
const merit = attempts
    ...
    .map((a, i) => ({
      rank: i + 1,
      // Fallback strings, never undefined — Firestore rejects undefined
      // outright (this exact crash), and a pre-existing attempt doc from
      // before the finalizeIfOverdue fix above could still be missing
      // studentName. This keeps analytics working today without needing
      // to manually patch that old data first.
      name: a.isGuest ? (a.guestName || "Guest") : (a.studentName || "Student"),
